...
public class ChatController : ControllerBase
{
    ...
    public async Task<IActionResult> Post(ChatRequest request, [FromQuery] string provider = "OpenAI")
    {
        ...
        var reply = await _chatService.SendMessageAsync(request.Prompt, request.ConversationId.ToString(), aiProvider);
        return Ok(new { message = reply.Message, provider = reply.Provider });
    }

    [HttpPost("regular")]
    public async Task<IActionResult> RegularChat(ChatRequest request, [FromQuery] string provider = "OpenAI")
    {
        if (string.IsNullOrWhiteSpace(request.Prompt))
            return BadRequest(new { error = "Prompt is required" });
        ...
        var reply = await _chatService.SendRegularMessageAsync(request.Prompt, request.ConversationId.ToString(), aiProvider);
        return Ok(new { message = reply.Message, provider = reply.Provider });
    }
